...
export function ChartToolbar({
  ...
  return (
    <div style={{
      ...
      {/* Chart type */}
      <span style={{ color: "#888", marginRight: 4 }}>图表:</span>
      {CHART_TYPES.map(ct => (
        <button key={ct.key} onClick={() => onChartTypeChange(ct.key)}
          style={{
            ...btnBase,
            background: chartType === ct.key ? "#fbbf24" : "#2a2a4a",
            color: chartType === ct.key ? "#000" : "#ccc",
      ...
      {/* Drawing tools */}
      <span style={{ color: "#888", marginRight: 4 }}>绘图:</span>
      {DRAWING_TOOLS.map(dt => (
        <button key={dt.key} onClick={() => onToolChange(activeTool === dt.key ? null : dt.key)}
          title={dt.label}
          style={{
            ...btnBase,
            background: activeTool === dt.key ? "#fbbf24" : "#2a2a4a",
            color: activeTool === dt.key ? "#000" : "#aaa",
      ...
      {drawingCount > 0 && (
        <>
          ...
          <button onClick={onClearDrawings} style={{
            ...btnBase, background: "#3a1a1a", color: "#ef4444",
          }}>
            清除全部 ({drawingCount})
          </button>
        </>
