...
> 目标：改善数据导入体验 + 回测可视化 + 预警系统 MVP

### 优先级一：通达信数据格式兼容导入
- 支持通达信 `.day`（日线）和 `.min`（分钟线）二进制格式直接读取
- 支持通达信 `T0002/export/` 导出目录批量导入
- 自动识别上交所/深交所代码前缀，自动填充 exchange 字段
- 这是用户获取的第一大障碍——大多数 A 股投资者已有通达信数据

### 优先级二：回测结果可视化增强
- 资金曲线图（Canvas 绘制，可缩放，叠加回撤阴影区域）
- 交易标记点（在 K线图上标注买卖点，用箭头/圆点）
- 回测报告导出（CSV 格式，含每笔交易详情）
- 多策略对比（同股票多策略结果并列展示）
