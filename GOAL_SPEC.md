...
> 版本 v0.9.0 | 2026-05-24 | Professional Terminal 主题 + K线训练模式 + 演示数据 + 交互补全
...
- 核心变化：
  - **背景色**：`#0C0C0C` 近纯黑（零蓝色调），减少长时间盯盘眼疲劳
  - **强调色**：从霓虹青 `#00D8FF` 改为低调金色 `#CCAA00`（Bloomberg 风格）
  - **涨跌色**：涨 `#26A69A`（低沉青绿）/ 跌 `#EF5350`（低沉红）
  - **边框**：从 `#1E293B` 改为 `#2A2A2A`（极浅，只做分区不做装饰）
  - **字体**：全局统一等宽字体，去除 Inter 等 UI 字体
  - **无渐变/无发光**：去除所有霓虹光效，回归扁平专业
- 设计原则：**"Data is the Hero"**——每像素服务于数据可读性
- CSS类名从 `cyber-*` 改为 `terminal-*`
- 保留无障碍模式（大字体 + 高对比度）不变

### 新增功能
- **K线训练模式**（复盘页"复盘模板"标签页）：
  - 加载60条历史日K线，逐根展示，隐藏后市走势
  - 用户判断涨跌方向，完成后对照真实走势评分
  - 准确率+正确/错误计数+四级评级（优秀/良好/一般/需加强）
- **演示股票数据预置**：
  - 首次启动自动写入3支A股演示数据：贵州茅台(600519)、宁德时代(300750)、招商银行(600036)
  - 每支约100条合成日线（非真实数据，仅供功能演示）
  - 仅DB为空时写入，不覆盖用户已导入数据
- **绘图选中/删除**：图表中点击绘图对象可选中，Delete键删除
- **组合分析PRO模块**：
  - 相关性矩阵：跨持仓收益率 Pearson r 矩阵，热力图可视化
  - 行业集中度：按行业维度分析持仓分布，支持自定义行业分类
  - VaR 风险价值：历史模拟法 95%/99% VaR 和 CVaR
  - 收益归因：按持仓盈亏贡献度分解，展示最大贡献/拖累标的

### 完善修改
- **版本号统一**：5处版本号统一至 v0.9.0（index.ts、SettingsPage、tauri.conf.json、Cargo.toml、package.json）
- **授权缓存优化**：试用状态纳入缓存层，减少并行命令时的数据库锁竞争
- **回测页策略提示**：禁用策略旁显示"(★ = PRO)"标识
- **交易日志改善**：移除静默 TODO 注释，错误改为 console.error 输出
