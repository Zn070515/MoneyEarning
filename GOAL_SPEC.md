...
> 版本 v0.10.0 | 2026-05-24 | 通达信导入 + 回测可视化增强 + 预警系统开发中
...
- 均线交叉预警（快线上穿/下穿慢线）
- 成交量异常预警（成交量 > N日均量 × M倍）
- 预警触发：Windows 系统通知 + 声音提示
- 预警列表管理（启用/禁用/删除）

### 优先级四：细节打磨
- 多图表同时打开（2×2 布局，独立时间周期）
- ME Script 编辑器增强（语法高亮、关键字补全）
- 持仓组合页支持 CSV 导入/导出
- 优化大数据量图表渲染（WebGL 加速散点图层）

### 优先级五：付费转化优化
- 试用到期后显示升级引导页（非阻断式，可关闭）
- 功能菜单中标注 PRO 专属标识
- 激活码输入框支持粘贴 + 自动格式化
- 增加"推荐给朋友"分享按钮（生成推荐链接）
